...
Game.prototype.rooms = async function gameRooms(title) {
  try {
    console.log(`Trying to find rooms for game ${util.inspect(this)}, room ${title}`)
    if (title) {
...
Game.prototype.players = async function gamePlayers(role) {
  try {
    console.log(`Trying to find players for game ${util.inspect(this)} with role ${role}`)
    if (role) {
...
Game.prototype.addRooms = async function addGameRooms(credentials, roomTitle, token, roles) {
  try {
    console.log(`Adding room ${roomTitle} to game ${this.gameId}`);
    const game = this;
    ...
    console.log(`ERROR (Game.addRooms): ${error}`)
  }
}
  ...
  try {
    ...
    let promises = rooms.map(async function(room) {
      ...
      if (check.length == 0) {
        ...
        console.log(`WARN (Game.addProxyRooms): Room ${room.title} already exists for game ${gameId}`);
        return check;
      }
    });
    let roomsOut = [];
    ...
    console.error(`ERROR (Game.addProxyRooms): ${err}`);
    return (err);
  }
}
