...
Room.prototype.addPlayer = async function addPlayer(credentials, player) {
  try {
    console.log(`Adding player ${util.inspect(player)} to room ${util.inspect(this)}`);
    var room = this;
...
Room.addToGame = async function addToGame(roomId, gameId) {
  console.log(`Adding room ${roomId} to game ${gameId}`);
  try {
    var query = `
...
Room.webhooks = async function webhooks(options) {
  try {
    console.log(`Searching for webhooks on room ${JSON.stringify(options)}`);
    var room = new Room(options);
    var query = `
      MATCH (r:${util.inspect(room)})--(w:Webhook)
      RETURN r,w`;
    var out = await db.db.query(query).getResults('w');
    console.log(`Found webhooks ${JSON.stringify(out)}`);
    return out;
  } catch (err) {
    console.log(err);
    return(err);
